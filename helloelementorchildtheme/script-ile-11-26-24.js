(function($){
    ...
    $(document).ready(function(){
		if($('body').hasClass('single-sfwd-lessons')){
			const iframe = $('.AnC-iFrame')[0];
			$(iframe).on('load', function() {                
				const iframeDoc = this.contentDocument || this.contentWindow.document;
				$(iframeDoc).on('click','a', function(e) {					
					if($(this).text().toLowerCase() == 'graduate'){
						e.preventDefault();
						if(!$(this).hasClass('clicked')){
							...
							if(sp_obj.course.id !== false && sp_obj.course.form !== false && sp_obj.lesson.id !== false){							
								$('.tclr-mark-complete-button').find('form.sfwd-mark-complete').addClass('popup_clicked');
								$('.complete_popup_form').css('display','flex');
							}else{
								$('.tclr-mark-complete-button').css('display','block');
								$('.tclr-mark-complete-button .learndash_mark_complete_button').removeClass('disabled').prop('disabled',false);
								$('.tclr-mark-complete-button .learndash_mark_complete_button').click();
							}
						}						
					}
					return false;
	...
	$(document).ready(function() {
		...
		$(document).on("click",".otpestatus .generateOtp",function() {
			$('.otp-input').each(function(index) {
	            $(this).val(''); // Set each input to the corresponding digit or leave empty
	        });	
	    });	
		...
		$(document).on('gform_post_render', function(event, formData, formId) {
			...
			for (let i = 0; i < 6; i++) {
			    ...
			    input.on('input', function () {
			        ...
			        $('#input_18_51_1').val($('.optobject .otp-input[data-index=0]').val() + $('.optobject .otp-input[data-index=1]').val() + $('.optobject .otp-input[data-index=2]').val() + $('.optobject .otp-input[data-index=3]').val() + $('.optobject .otp-input[data-index=4]').val() + $('.optobject .otp-input[data-index=5]').val());
